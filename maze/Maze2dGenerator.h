...
public:
    ...
    std::string measureAlgorithmTime(int width, int height) {
        auto t1 = std::chrono::high_resolution_clock::now();
        Generte(width,height);
...
public:
    ...
    int IsInBounds(int x, int y, Maze2d *mz) { // Returns "true" if x and y are both in-bounds.
        if (x < 0 || x >= (mz->get_height() - 1)) return false;
        if (y < 0 || y >= (mz->get_width() - 1)) return false;
        return true;
    }

    void Visit(int x, int y, Maze2d *mz) {
...
// on the next direction I wish to try.
            ...
            int x2 = x + (dx << 1);
            int y2 = y + (dy << 1);
            if (IsInBounds(x2, y2, mz)) {
                if (mz->get_grid()[x2][y2] == '#') {
// (x2,y2) has not been visited yet... knock down the
// wall between my current position and that position
                    mz->set_char_in_grid(x2 - dx,y2 - dy , ' ');
